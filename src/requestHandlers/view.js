 ...
 * @param {object} response 
 */
function reqDate(request, response){
    console.log("Request handler 'date' is processing.");
...
function reqDisplay(request, response, result){
    ...
    const fd = fs.openSync('../data/itemRecord.csv', 'r');
    fs.readFileSync(fd,'utf8');

    var readData = (fs.readFileSync('../data/itemRecord.csv')).toString();
    ...
    tableData = "<!DOCTYPE html>" + 
            ...
            "<body>" +
            "<h1>List of Items</h1>";
            tableData +=`<h2 style="color:blue;text-align:center;">Number items in the list: ${count}</h2>`+
            "<table>" + 
            "<tr>";
    ...
    function myTableData()
    {
        ...
        for(var i=1; i<row.length - 1; i++){
            ...
            var dataArr = data[2].split(delimiter);
            for(var interval=0; interval<row.length; interval += 3){
                var intervalData = dataArr[interval];
                if(`${result.date}` === intervalData){
